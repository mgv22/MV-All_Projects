...
model_4_data <- model_df[, .(TotalSF, OverallQual, logSalePrice, SalePrice, TotalBsmtSF)]

data <- data.frame(model_df$SalePrice, model_df$logSalePrice, model_df$TotalSF, model_df$OverallQual, model_df$TotalBsmtSF)
data$residuals <- residuals(model_4)
outlier_threshold <- 0.002
outliers <- data[abs(data$residuals) > outlier_threshold, ]
data<- data[!273, !707, !1567]

model_4_new<- lm(
    model_df$logSalePrice ~ model_df$TotalSF + model_df$OverallQual + model_df$TotalBsmtSF
  )

ols_plot_cooksd_bar(model_4_new)

data<- data[!62, !74, !1617]
...
```{r}
regression_diagnostics(model_4)
